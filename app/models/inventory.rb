class Inventory < ActiveRecord::Base
  ...
  scope :need_adjustment, includes(:item).includes(:location)\
    .where(:ao_adj.not_eq => 0).where(:items => {:is_active => true, :from_al => true, :is_lotted => false}).where(:locations => {:from_al => true}).report_valid.order([{:locations => :code.asc}, {:items => :code.asc}])
  scope :need_manually_adj, includes(:item).includes(:location)\
    .where({:items => (:from_al.eq % false | :is_lotted.eq % true | :is_active.eq % false)} | {:locations => {:from_al => false}}).where(:result_qty.gt => 0).report_valid
