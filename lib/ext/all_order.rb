...
module ALL_ORDER
  class Import
    ...
    def self.items check
      ...
      self.xls(book, [{:list => check.items.where(:from_al.eq % false || :data_changed.eq % true), :symbols => [:code, :group_name, nil, nil, nil, :description, nil, nil, nil, nil, nil, nil, :cost, nil, nil, nil, :adj_max_quantity]}], :name => "ItemInventory")
    end
